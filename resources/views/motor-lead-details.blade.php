...
@if($val->Count!='0' and $val->url!='')
      
      <a href="#">
       <div class="col-md-2 month1 col-xs-6 col-sm-6 {{$set_class}}">
       <span class="count">
       <a href="{{url('motor-lead-alldetails')}}/{{$val->FBAID}}/{{$val->month_no}}">{{$val->Month_Name}} <br>({{$val->Count}})
       </a>
       </span>
       </div>
      </a> 
      ...
      <a href="#">
        <div class="col-md-2 month1 col-xs-6 col-sm-6 {{$set_class}}">
          <a href="#" data-toggle="modal" data-target="#spassword" onclick='load_modal("count_{{$val->Month_Name}}")'>{{$val->Month_Name}} <br>(<span id="count_{{$val->Month_Name}}">{{$val->Count}}</span>)
          </a>
        </div>
      </a> 
...
<div style="margin-left:12.4%;margin-top: 32px;font-weight: bolder;">Leads generated from Syncing of contacts: {{$val->Contactscount}} leads from {{$val->newcount}} contacts synced</div>
<div style="margin-left:12.4%;margin-top: 13px;font-weight: bolder;">Manual leads generated: {{$val->manullycount}}</div>
