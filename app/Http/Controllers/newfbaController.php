...
use Excel;
   class newfbaController extends CallApiController {
    ...
    $columns = array('FBAID','FullName','CreatedDate','City','State','Zone','CustID','PospStatus','RRM','FieldSales','ERPID','AppSource','POSPNo','PospName','PayStat','Type');
...
//               $query=[];
//               $query=DB::select('call fbaList_export(0)');
//              // $query=DB::select("call fbaList_export(0,'2019-01-10','2019-02-15')");
...
// }
           ...
           public function update_fba_list($fbaid){
                ...
                if(count($update) > 0){
                      return view('dashboard.update-fba-list',['data'=>$update[0],'subfba'=>$subfba,'Hierarchy'=>$Hierarchy,'hierarchypr'=>$hierarchypr,'recruiter'=>$recruiter,'trainingopsuid'=>$trainingopsuid,'fieldsalesuid'=>$fieldsalesuid,'clusterheaduid'=>$clusterheaduid,'stateheaduid'=>$stateheaduid,'zonalheaduid'=>$zonalheaduid,'sourceupdate'=>$sourceupdate,'lastbuss'=>$lastbuss,'calllogs'=>$calllogs,'refererby_recruiter'=>$refererby_recruiter]);
                    }
           else{
          ...
          public function UpdatePospnonew($id)
{
   try{
    ...
    $types = array("1"=>"FBA","2"=>"POSP","3"=>"T-POS");
