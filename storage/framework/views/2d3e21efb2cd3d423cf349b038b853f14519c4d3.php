<script src="<?php echo e(URL::to('javascripts/bootstrap-multiselect.js')); ?>"></script>
  <link rel="stylesheet" href="<?php echo e(URL::to('stylesheets/bootstrap-multiselect.css')); ?>" />
...
$(document).ready(function(){
 ...
 $("#menu_group_id_").click(function(event){  event.preventDefault(); 	  
       ...
       if($('#menu__group').val()!=0  ){ 
            $.post("<?php echo e(url('menu-group-save')); ?>",$('#menu_group_id_from').serialize())
             .done(function(data){ 
             	// console.log(data);
                 if(data==0){
                 window.location.href = "<?php echo e(url('menu-group')); ?>";
                 }else{
  ...
  $("#menu_list_ID").click(function(event){  event.preventDefault(); 	 
            ...
            $.post("<?php echo e(url('menu-add')); ?>",$('#menu_list_from').serialize())
             .done(function(data){ 
             	// console.log(data);
                 if(data==0){
                     
                 window.location.href = "<?php echo e(url('lead-up-load')); ?>";
                 }else{
