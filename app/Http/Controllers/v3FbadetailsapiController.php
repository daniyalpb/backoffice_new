...
use URL;
use Mail;
class v3FbadetailsapiController extends ApiController
{
	...
	public function getcrmfollowup(Request $req)
	{
       //print_r($req->all());exit();
		...
		if(!empty($assign) && $assign[0]->followup_internalteam != null || $assign[0]->followup_internalteam != ''){	 
		 ...
		 $data=DB::select('call new_crm_insert_from_app(?,?,?,?,?,?,?,?,?,?,?)',array(
              $req->disposition_id,
              $req->Uid,
              $req->crm_id,
              $req->fbamappin_id,             
...
public function Break_in_notification_count(Request $req){
	...
	public function connectionresult()
	{
		...
		public function get_crm_disposition(Request $req){
		 	 $data=DB::select('call new_get_crm_disposition(?)',array($req->Uid));
		  	if (!empty($data)) {
