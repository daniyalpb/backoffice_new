...
class statecitywiseprofileController extends CallApiController 
{
	...
	public function updateuid(Request $req){
		...
		if($req->chekpin==0 and isset($req->mapfba)){
			$post_array = $req->all();
		   // $PinCode_array = $post_array['txtpincode'];
			$PinCode_array = explode(',',$post_array['txtpincode']);
	...
	// DB::select('call update_override_fba_crm_new(?,?,?)',array($post_array['eprofile'],$PinCode,$post_array['pname']));
		...
		else if($req->chekpin==0){

			$post_array = $req->all();
			$PinCode_array = explode(',',$post_array['txtpincode']);
			foreach($PinCode_array as $txtpincode){
				DB::select('call update_pincode_manually(?,?,?)',array($post_array['eprofile'],$txtpincode,$post_array['pname']));
			}

                 // echo "rferer";
		}

		else if($req->chekpin==1){
			$post_array = $req->all();
		...
		else if($req->chekpin==1 and isset($req->mapfba)){
			if($req->chekpin==0 and isset($req->mapfba)){
				$post_array = $req->all();
				$PinCode_array = $post_array['txtpincode'];
				foreach($PinCode_array as $PinCode){
