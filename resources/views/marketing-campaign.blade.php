...
<body>
	<div style ="margin:0 auto; background-color:#482fb1;padding-bottom:10px;">
		<div><img style="width:100%; height:100%;" src="{{url('images/head_.jpg')}}"/></div>  

		@foreach($campignimagedata as $val)
		<a href="{{$val->Description}}" target="_blank">
		...
		@foreach($campignurldata as $val) 
		<a href="{{$val->Description}}" target="_blank">
		<div style="width:80%;background-color:#ffffff;padding:10px; margin:10px auto;border:2px solid #cccaca;border-radius:5px;box-shadow:0 0 8px 0px #131212;">
			<h4>{{$val->Title}}</h4>
			{{$val->shortdescription}} 
			<!-- <a href="{{$val->Description}}" >Short Description</a>  -->
		</div>
		</a>
		@endforeach

		@foreach($campignpdfdata as $val) 
		<a href="{{$val->Description}}" target="_blank">
		<div style="width:80%;background-color:#ffffff;padding:10px; margin:10px auto;border:2px solid #cccaca;border-radius:5px;box-shadow:0 0 8px 0px #131212;">
			<h4>{{$val->Title}}</h4>
			{{$val->shortdescription}}
		...
		@foreach($campignhtmldata as $val)
