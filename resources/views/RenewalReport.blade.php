...
<form method="Post" action="{{url('Renewal-report')}}"> 
  {{ csrf_field() }}
   <p>From Date</p>
    ...
    <tbody>
      @isset($data)
        ...
        <tr>
            ...
            <td><a onclick="getrenewaldetails({{$val->FBAID}})">{{$val->count}}</a></td>        
        </tr>
        @endforeach
        @endisset
    </tbody>
...
<!-- Modal -->
  <div class="modal fade" id="moredetails" role="dialog">
    <div class="modal-dialog modal-lg" style="width: 1240px;">
     ...
     @isset($date)
<input type="hidden" name="fromdate" id="fromdate" value="{{$date['fromdate']}}">
<input type="hidden" name="todatee" id="todatee" value="{{$date['todate']}}">
