...
<div class="col-md-8 col-md-offset-2 box-shadow">
	...
	@if(count($today_array) > 0)
		@foreach($today_array as $key => $value)
			<ul>
			@foreach($value as $training_id => $training_value)
			<li id="li_upcoming_{{ $training_id }}" data-attn="{{ check_attn_time($training_value['training_duration'], $training_value['training_start_time'],  $training_value['training_date'], $training_value['training_is_accepted']) }}" data-val="{{ $training_id }}" data-toggle="modal" data-target="#myModal" onclick="show_modal(this.id)">
				<b><p class="text-muted1">{{ $training_value['training_name'] }}</p></b>
			</li>
			@endforeach
			</ul>
		@endforeach
  ...
  <div class="tab-content">
    ...
    @if(count($upcoming_array) > 0)
    @foreach($upcoming_array as $key => $value)
      <div class="months">
		<h4>{{-- {{ $key }} --}}</h4>
		<ul>
		@foreach($value as $training_id => $training_value)
		<li style="list-style-type:none" id="li_upcoming_{{ $training_id }}" data-attn="{{ check_attn_time($training_value['training_duration'], $training_value['training_start_time'], $training_value['training_date'], $training_value['training_is_accepted']) }}" data-val="{{ $training_id }}" data-toggle="modal" data-target="#myModal" onclick="show_modal(this.id)">
			<b>{{ $training_value['training_date'] }}</b>
			<p class="text-muted">{{ $training_value['training_name'] }}</p>
			@if($training_value['training_is_accepted'] == "1")
		        <font color='black'>Accepted Status</font> = <span id="span_accepted_status_{{ $training_id }}"><font color='green'><b>Accepted</b></font></span>
			@elseif($training_value['training_is_accepted'] == "0")
	            <font color='black'>Accepted Status</font> = <span id="span_accepted_status_{{ $training_id }}"><font color='red'><b>Rejected</b></font></span>
			@elseif($training_value['training_is_accepted'] == "" or $training_value['training_is_accepted'] == null)
				<font color='black'>Accepted Status</font> = <span id="span_accepted_status_{{ $training_id }}"><font color='blue'><b>Pending</b></font></span>
			@else
				<font color='black'>Accepted Status</font> = <span id="span_accepted_status_{{ $training_id }}"></span>
			@endif
		</li>
		@endforeach
		</ul>
   ...
   {{--  <div id="menu1" class="tab-pane fade">
      @if(count($past_array) > 0)
      @foreach($past_array as $key => $value)
      <div class="months">
		<h4>{{ $key }}</h4>
		<ul>
		@foreach($value as $training_id => $training_value)
		<li>
			<b>{{ $training_value['training_date'] }}</b>
			<p class="text-muted">{{ $training_value['training_name'] }}</p>
				@if($training_value['training_is_accepted'] == "" or $training_value['training_is_accepted'] == "1")
...
<script src="{{ url('/javascripts/jquery_v3.3.1.min.js') }}"></script> 
<script src="{{ url('/javascripts/bootstrap/bootstrap.min.js') }}"></script> 
<script>
function show_modal(this_id){
	...
	$.ajax({
		type : 'get',
		data : {'training_id':training_id , 'fbaid' : {{ $fbaid }} },
		url : "{{ URL::to('/fill-modal-training-data') }}",
		success : function(response){
			...
			$.each(data ,function(key , value){
				...
				if((key == "is_accepted") && (value == "" || value == null) && (data.training_date != "{{ date("Y-m-d") }}")){
				...
				if(key == "is_accepted" && value == "0" && (data.training_date != "{{ date("Y-m-d") }}")){
					$("#btn_show_hide_reject_training").html('');
					$("#invitation_response").html('<font color="red">You Rejected this Training Invitation.</font>');
				}
				if(key == "is_accepted" && value == "1" && (data.training_date != "{{ date("Y-m-d") }}")){
					$("#btn_show_hide_accept_training").html('');
...
if(!($("#reject_reason_div").is(":visible"))){
	...
	if($("#reject_reason").val() == ""){
	    ...
	    if(sure){
	    	...
	    	$.ajax({
	    		type : 'get',
				data : {'training_id':training_id,'reject_reason':reject_reason , 'fbaid' : {{ $fbaid }} },
				url : "{{ URL::to('/upd-training-rejected-status') }}",
				success : function(response){
					var data = JSON.parse(response);
...
function upd_training_accepted_status(training_id){
	$.ajax({
		type : 'get',
		data : {'training_id':training_id , 'fbaid' : {{ $fbaid }} },
		url : "{{ URL::to('/upd-training-accepted-status') }}",
		success : function(response){
			var data = JSON.parse(response);
			$("#invitation_response").html(data.alert_msg);
			$("#btn_show_hide_accept_training").html('');
...
function upd_training_attendance(training_id){
	$.ajax({
		type : 'get',
		data : {'training_id':training_id, 'fbaid' : {{ $fbaid }} },
		url : "{{ URL::to('/upd-training-attendance') }}",
		success : function(response){
			var data = JSON.parse(response);
